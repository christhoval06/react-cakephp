...
class PaypalRedirectException extends CakeException {}
	...
	class Paypal {
		...
		**/
		public function getVerifiedStatus($email) {
			...
			if(in_array($parsed['responseEnvelope']['ack'], array('Success', 'SuccessWithWarning'))) {
				...
				throw new PaypalException(__d('paypal', 'Error executing paypal request: %s', $message));
			} else {
		...
		**/
		...
		public function getRecurringPaymentsProfileDetails($profileId) {
			try {
				$nvps = $this->buildGetRecurringPaymentsProfileDetailsNvp($profileId);
				// Add/overite, we now habe our final NVPs
				$finalNvps = array_merge($nvps, array());
				...
				if (isset($parsed['ACK']) && in_array($parsed['ACK'], array('Success', 'SuccessWithWarning')))  {
					...
					$exception = new PaypalException($this->getErrorMessage($parsed));
					$exception->parsed = $parsed;
					throw $exception;
				}
				else {
					$exception = new PaypalException(__d('paypal', "There was an error retrieving recurring payment profile details."));
					$exception->parsed = $parsed;
					throw $exception;
				}
			}
			catch (SocketException $e) {
				throw new PaypalException(__d('paypal', 'There was a problem retrieving recurring payment profile, please try again.'));
			}
		}

		public function createRecurringPaymentsProfile($billing) {
			try {
				$nvps = $this->buildCreateRecurringPaymentsProfileNvp($billing);
				// Add/overite, we now habe our final NVPs
				$finalNvps = array_merge($nvps, array());
				...
				if (isset($parsed['ACK']) && in_array($parsed['ACK'], array('Success', 'SuccessWithWarning')))  {
					...
					$exception = new PaypalException(__d('paypal', "There was an error completing the recurring payment transaction"));
					$exception->parsed = $parsed;
					throw $exception;
				}
			}
			catch (SocketException $e) {
				throw new PaypalException(__d('paypal', 'There was a problem processing the recurring payment transaction, please try again.'));
			}
		}

		public function manageRecurringPaymentsProfileStatus($profile) {
			try {
				$nvps = $this->buildManageRecurringPaymentsProfileStatusNvp($profile);
				$finalNvps = array_merge($nvps, array());
				if (!$this->HttpSocket) {
					$this->HttpSocket = new HttpSocket();
				}
				...
				if (isset($parsed['ACK']) && in_array($parsed['ACK'], array('Success', 'SuccessWithWarning')))  {
					...
					$exception = new PaypalException(__d('paypal', "There was an error completing the manage recurring payments profile status."));
					$exception->parsed = $parsed;
					throw $exception;
				}
			}
		...
		public function updateRecurringPaymentsProfile($profile) {
			try {
				$nvps = $this->buildUpdateRecurringPaymentsProfileNvp($profile);
				$finalNvps = array_merge($nvps, array());
				if (!$this->HttpSocket) {
					$this->HttpSocket = new HttpSocket();
				}
		...
		public function doExpressCheckoutPayment($order, $token , $payerId) {
			try {
				...
				if (isset($parsed['TOKEN']) && isset($parsed['ACK']) && in_array($parsed['ACK'], array('Success', 'SuccessWithWarning')))  {
					...
					$exception = new PaypalException(__d('paypal', "There was an error completing the payment"));
					$exception->parsed = $parsed;
					throw $exception;
				}
			} catch (SocketException $e) {
	...
	**/
	...
	/**
	 * Build request to send to paypal to retrieve recurring payment profile informations.
	 * @param  string $profileId Profile id for which retrieve informations.
	 * @return array Formatted array of Paypal NVPs.
	 */
	public function buildGetRecurringPaymentsProfileDetailsNvp($profileId) {
		$nvps = array(
	...
	/**
	 * Build request to send to paypal to activate recurring payment.
	 * @param  array $order Takes an array order.
	 * @return array Formatted array of Paypal NVPs for CreateRecuringPaymentsProfile.
	 * @author Roberto Conte Rosito
	 */
	public function buildCreateRecurringPaymentsProfileNvp($profile) {
		if (!isset($profile['token'])) {
			throw new PaypalException(__d('paypal', 'You must pass a valid token (token)'));
		}
		if (!isset($profile['payer_id'])) {
			throw new PaypalException(__d('paypal', 'You must pass a valid payer id (payer_id)'));
		}
		if (!isset($profile['start_date'])) {
			throw new PaypalException(__d('paypal', 'You must pass a valid start date (start_date) for recurring payment'));
		}
		if (!isset($profile['desc'])) {
			throw new PaypalException(__d('paypal', 'You must pass a valid description (desc) for recurring payment'));
		}
		if (!isset($profile['period'])) {
			throw new PaypalException(__d('paypal', 'You must pass a valid billing period (period) for recurring payment'));
		}
		else {
			$periods = array('Day', 'Week', 'SemiMonth', 'Month', 'Year');
			if(!in_array($profile['period'], $periods)) {
				throw new PaypalException(__d('paypal', 'You must pass a valid billing period (period) for recurring payment'));
			}
		}
		if (!isset($profile['frequency'])) {
			throw new PaypalException(__d('paypal', 'You must pass a valid billing frequency (frequency) for recurring payment'));
		}
		if (!isset($profile['currency_code'])) {
			throw new PaypalException(__d('paypal', 'You must pass a valid currency code (currency_code)'));
		}
		if (!isset($profile['country_code'])) {
			throw new PaypalException(__d('paypal', 'You must pass a valid country code (country_code)'));
		}
		if (!isset($profile['amount'])) {
			throw new PaypalException(__d('paypal', 'You must pass a valid amount (amount) for recurring payment'));
		}
		...
		$nvps = array(
			...
			'SIGNATURE' => $this->nvpSignature,
			'TOKEN' => $profile['token'],
			'PAYERID' => $profile['payer_id'],
			'PROFILESTARTDATE' => $profile['start_date'],
			'DESC' => $profile['desc'],
			'BILLINGPERIOD' => $profile['period'],
			'BILLINGFREQUENCY' => $profile['frequency'],
			'AMT' => $profile['amount'],
			'CURRENCYCODE' => $profile['currency_code'],
			'COUNTRYCODE' => $profile['country_code'],
			'MAXFAILEDPAYMENTS' => isset($profile['max_failed_payments'])
				? $profile['max_failed_payments']
				: 1
		);
		if (isset($profile['init_amount'])) {
			$nvps['INITAMT'] = $profile['init_amount'];
		}
		if (isset($profile['init_amount_failed_action'])) {
			$acceptedValues = array('ContinueOnFailure', 'CancelOnFailure');
			if(in_array($profile['init_amount_failed_action'], $acceptedValues)) {
				$nvps['FAILEDINITAMTACTION'] = $profile['init_amount_failed_action'];
			}
			else {
				throw new PaypalException(__d('paypal', 'Invalid failed action for initial amount payment'));
			}
		}
		if (isset($profile['custom'])) {
			$nvps['CUSTOM'] = $profile['custom'];
		}
		return $nvps;
	}

	/**
	 * Create npv's request necessary to work with recurring payment profile status manager.
	 * @param  array $profile profile informations {profile_id, action}
	 * @return array containing profile npv's request.
	 * @author Roberto Conte Rosito
	 */
	public function buildManageRecurringPaymentsProfileStatusNvp($profile) {
		if (!isset($profile['profile_id'])) {
			throw new PaypalException(__d('paypal', 'The profile_id prop is required.'));
		}
		if (!isset($profile['action'])) {
			throw new PaypalException(__d('paypal', 'The action prop is required.'));
		}
		else {
			$action = $profile['action'];
			$validActions = array('Cancel', 'Suspend', 'Reactive');
			if (!in_array($action, $validActions)) {
				throw new Exception(__d('paypal', 'Invalid action (Cancel|Suspend|Reactive allowed).'));
			}
		}
	...
	public function buildUpdateRecurringPaymentsProfileNvp($request) {
		if (!isset($request['profile_id'])) {
			throw new PaypalException(__d('paypal', 'The profile_id prop is required.'));
		}
		if (!isset($request['args'])) {
			throw new PaypalException(__d('paypal', 'The args prop is required.'));
		}
		$nvps = array(
		...
		return array_merge($nvps, $request['args']);
	}
	...
	public function buildExpressCheckoutNvp($order) {
		...
		// Billing type different from standard?
		if (isset($order['billing_type'])) {
			$nvps['L_BILLINGTYPE0'] = $order['billing_type'];
		}
		if (isset($order['billing_type_description'])) {
			$nvps['L_BILLINGAGREEMENTDESCRIPTION0'] = $order['billing_type_description'];
		}
