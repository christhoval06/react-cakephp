<?php
  ...
  class WhoisComponent extends Component {
    ...
    public function resolve($s) {
      if( $this->validateIP($s)) {
        return $this->lookupIP($s);
      }
      else if($this->validateDomain($s)) {
        return $this->lookupDomain($s);
      }
      else {
    ...
    private function lookupDomain($domain) {
      ...
      if(is_null($server)) {
        return array(
          'error' => true,
          'message' => "Error: no appropriate whois server found for $domain domain!"
        );
      }
      ...
      if(!$result) {
        return array(
          'error' => true,
          'message' => "Error: no results retrieved from $server server for $domain domain!"
        );
      }
      else {
        if(isset($result['Whois Server:'])) {
          $secondary = $result['Whois Server:'];
          if($secondary) {
            $result = $this->queryWhoisServer($secondary, $domain);
            $server = $secondary;
          }
        }
        return array('error' => false, 'response' => $result);
      }
    ...
    private function queryWhoisServer($whoisserver, $domain) {
        ...
        // whois.verisign-grs.com needs to be proceeded by
        // the keyword "domain ", otherwise it will return any
        // result containing the searched string.
      ...
      $res = array();
    	if((strpos(strtolower($out), "error") === FALSE) &&
         (strpos(strtolower($out), "not allocated") === FALSE)) {
    		$rows = explode("\n", $out);
        $prev = null;
        $jump = array('#','%','*');
    		foreach($rows as $row) {
